...
const Utils = {
    ...
    // Return first matching element for a list of selector alternatives
    find(...selectors) {
...
const UIManager = {
    ...
    setupBackToTop() {
        // Support both legacy and current id names
        const backToTop = Utils.find('#back-to-top', '#to-top-button', '.to-top');
        if (backToTop) {
...
const CSRFManager = {
    ...
    async fetchToken() {
        try {
            const resp = await fetch('/get_csrf_token.php', { credentials: 'same-origin' });
            ...
            console.warn('CSRFManager.fetchToken error:', err);
            this.token = null;
            throw err;
        }
    }
...
class RadioAdamowoApp {
    ...
    async init() {
        try {
            ...
            // Attempt to fetch CSRF token to enable secure server interactions
            try {
                await CSRFManager.fetchToken();
                console.log('CSRF token loaded');
            } catch (e) {
                console.warn('Could not load CSRF token; comment posting will fallback to localStorage.');
            }
